...
const STEP_CONFIG: StepConfig[] = [
  {
    ...
    description: " Votre profil professionnel (Sélectionnez toutes les réponses possibles)",
    question: " Votre profil professionnel (Sélectionnez toutes les réponses possibles)",
  },
  {
  ...
  },
  {
    ...
    description: " Quel est votre objectif principal en suivant nos formations ? (Choix multiples)",
    question: " Quel est votre objectif principal en suivant nos formations ? (Choix multiples)",
  },
  {
...
function sanitizeStoredData(raw: unknown): { formData: FormData; step: number } | null {
  ...
  const safeData: FormData = {
    profile: Array.isArray(storedForm.profile) ? storedForm.profile.filter((item) => typeof item === "string") : [],
    ...
    goals: Array.isArray(storedForm.goals) ? storedForm.goals.filter((item) => typeof item === "string") : [],
  ...
  const safeStep = Number.isFinite(storedStep) ? Math.min(Math.max(storedStep, 0), TOTAL_STEPS - 1) : 0
...
export default function TrainingSignupWizard() {
  ...
  const stepErrors = React.useMemo(() => getStepErrors(currentStep, formData), [currentStep, formData])
  ...
  const toggleMultiSelect = (key: "profile" | "interests" | "goals", value: string) => {
    setFormData((prev) => {
  ...
  return (
    <Card className="bg-surface/80 border-border/80 shadow-md backdrop-blur">
      <CardHeader className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-accent">Inscription Formations</p>
      ...
      <CardContent>
        <div key={currentStep} className="animate-fade-in-up">
          {currentStep === 0 && (
            <fieldset className="space-y-4" aria-invalid={showErrors && !!stepErrors.profile}>
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        id={id}
                        checked={checked}
                        onCheckedChange={() => toggleMultiSelect("profile", option)}
              ...
              {showErrors && stepErrors.profile ? (
                <p className="text-xs text-destructive">{stepErrors.profile}</p>
              ) : null}
            </fieldset>
          )}

          {currentStep === 1 && (
            <fieldset className="space-y-4" aria-invalid={showErrors && !!stepErrors.interests}>
              <legend className="sr-only">{stepMeta?.question}</legend>
              <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        id={id}
                        checked={checked}
                        onCheckedChange={() => toggleMultiSelect("interests", option)}
              ...
              {showErrors && stepErrors.interests ? (
                <p className="text-xs text-destructive">{stepErrors.interests}</p>
              ) : null}
            </fieldset>
          )}

          {currentStep === 2 && (
            <fieldset className="space-y-4" aria-invalid={showErrors && !!stepErrors.level}>
              <legend className="sr-only">{stepMeta?.question}</legend>
              <RadioGroup
          ...
          {currentStep === 3 && (
            <fieldset className="space-y-4" aria-invalid={showErrors && !!stepErrors.goals}>
              <legend className="sr-only">{stepMeta?.question}</legend>
              <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        id={id}
                        checked={checked}
                        onCheckedChange={() => toggleMultiSelect("goals", option)}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  aria-describedby={showErrors && stepErrors.name ? "error-name" : undefined}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  aria-describedby={showErrors && stepErrors.phone ? "error-phone" : undefined}
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input
                  ...
                  aria-describedby={showErrors && stepErrors.email ? "error-email" : undefined}
          ...
          {isConfirmation && (
            <div className="space-y-4 text-sm text-muted">
              <p className="text-base font-semibold text-foreground">? Inscription prise en compte</p>
              <p>Merci pour votre intérêt.</p>
              <p>
                Notre équipe vous contactera prochainement pour vous transmettre les informations détaillées
        ...
        {isConfirmation ? (
          ...
          <>
            <Button
              ...
              className={cn("w-full sm:w-auto", currentStep === 0 && "invisible sm:visible")}
            >
              Précédent
            </Button>
            {isLastFormStep ? (
